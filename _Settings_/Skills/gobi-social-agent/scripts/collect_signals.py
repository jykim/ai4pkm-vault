...
Usage:
  ...
  python3 _LifeOS_/Skills/gobi-social-agent/scripts/collect_signals.py --hours 24 --days 3
...
# Heuristic topic keywords (used for "similar-topic" dedup)
_TOPIC_KEYWORDS = [
    # EN
    "inbox",
    "bucket",
...
def load_recent_reply_topics(state_path: Path, window_hours: int) -> list[dict]:
    if not state_path.exists():
        return []
    try:
    ...
    items = state.get("recent_reply_topics") or []
...
def topic_signature(text: str) -> set[str]:
    t = (text or "").lower()
    return {kw for kw in _TOPIC_KEYWORDS if kw.lower() in t}
...
def is_similar_topic(signal: dict, recent: dict) -> bool:
    # Only consider same space + same author (tight rule; avoids over-skipping)
    if (signal.get("space_slug") or "") != (recent.get("space_slug") or ""):
        return False
    if (signal.get("author") or "") != (recent.get("author") or ""):
        return False

    s_text = f"{signal.get('title','')}\n{signal.get('preview','')}"
    r_sig = set(recent.get("topic_sig") or [])
    s_sig = topic_signature(s_text)

    # Primary: keyword signature overlap
    if r_sig and s_sig and (r_sig & s_sig):
        return True

    # Fallback: English token overlap (for EN-only posts)
    s_words = word_tokens(s_text)
    r_words = word_tokens(str(recent.get("title") or ""))
    if not s_words or not r_words:
        return False
    inter = len(s_words & r_words)
    union = len(s_words | r_words)
    return union > 0 and (inter / union) >= 0.2
...
def _read_vault_slug_from_settings(vault: Path) -> str | None:
    """Best-effort fallback for self vault slug.

    Some environments return `vaultSlug: null` from `gobi auth status`.
    When that happens, we can still infer the vault slug from the local
    project settings file created by `gobi vault init`.
    """
    settings = vault / ".gobi" / "settings.yaml"
    ...
    for line in text.splitlines():
        # naive YAML parse: vaultSlug: <slug>
        if line.strip().startswith("vaultSlug:"):
            _, _, val = line.partition(":")
            slug = val.strip().strip('"').strip("'")
            return slug or None
    return None


def _self_vault_slug(vault: Path) -> str | None:
    data = _gobi_json(["auth", "status"], timeout=5)
    if data:
        slug = data.get("vaultSlug")
        if slug:
            return slug
    # fallback: local project settings
    return _read_vault_slug_from_settings(vault)
...
def fetch_live_community_signals(
    ...
    recent_reply_topics: list[dict],
    per_space_limit: int = 30,
) -> tuple[list[dict], int]:
    """Pull recent space-post candidates from Gobi API for Reply targeting.

    Returns space-posts authored by *others*, created since `cutoff`,
    not yet in the user's `replied_to_posts` state list.

    Additionally applies a heuristic "similar-topic" filter:
    - same space + same author
    - recent reply within window
    - topic signature overlap (or EN token overlap)

    Returns: (signals, similar_skipped_count)
    """
    signals: list[dict] = []
    similar_skipped = 0
    spaces = _list_spaces()
    if not spaces:
        return signals, similar_skipped
    ...
    for space in spaces:
        ...
        for post in _space_feed(slug, per_space_limit):
            ...
            # Similar-topic dedup (same author + same space within window)
            if any(is_similar_topic(sig, r) for r in recent_reply_topics):
                similar_skipped += 1
                continue
...
def main() -> int:
    ...
    parser.add_argument("--self-vault-slug", type=str, default=None,
                        help="override self vault slug for filtering own posts (ask user for this value)")
    parser.add_argument("--api-per-space-limit", type=int, default=30,
                        help="max posts to pull from each space feed")
    parser.add_argument("--similar-window-hours", type=int, default=24,
                        help="heuristic similar-topic dedup window (same author+space)")
    ...
    state_path = args.state or (vault / "_LifeOS_/Skills/gobi-social-agent/state/_state.json")
    used = load_seen(state_path)
    replied = load_replied_post_ids(state_path)
    recent_topics = load_recent_reply_topics(state_path, window_hours=args.similar_window_hours)
    ...
    similar_skipped = 0
    if not args.no_api:
        self_slug = args.self_vault_slug or _self_vault_slug(vault)
        if self_slug is None:
            api_status = "auth_failed"
        else:
            api_signals, similar_skipped = fetch_live_community_signals(
                cutoff=comm_since,
                self_slug=self_slug,
                replied_post_ids=replied,
                recent_reply_topics=recent_topics,
    ...
    output = {
        ...
        "community_counts": {
            ...
            "similar_topic_skipped": similar_skipped,
            "recent_reply_topics_considered": len(recent_topics),
        },
