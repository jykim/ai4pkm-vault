...
- **My signals** (지난 24h 수정): `_LifeOS_/Journal/`, `AI/Roundup/`, `AI/Summary/`, `AI/Analysis/`, `AI/Events/`, `AI/Sharable/`, `_Outbox_/BrainUpdates/`
...
- **State**: `_LifeOS_/Skills/gobi-social-agent/state/_state.json` — 마지막 실행 시각, 이미 사용한 소스 히스토리
...
```bash
python3 _LifeOS_/Skills/gobi-social-agent/scripts/collect_signals.py \
  --hours 24 --days 3 --self-vault-slug <SELF_VAULT_SLUG>
```

처음 실행할 때 사용자에게 아래를 확인:
- `SELF_VAULT_SLUG` (예: `jyk`) — 내 포스트(own post) 필터링에 필요
- (선택) 주로 모니터링할 space slug 목록 (없으면 `gobi space list`로 전부 순회)
...
- `--state <path>`: state 파일 경로 (기본: `_LifeOS_/Skills/gobi-social-agent/state/_state.json`)
...
> 테스트 모드: **2시간 쿨다운(throttle)은 비활성화**. heartbeat(30분)마다 실행돼도 쿨다운 때문에 SKIP하지 않는다.
...
**가장 강한 내 시그널 1개** 선정:
- 우선순위: `AI/Analysis/` > `AI/Summary/` > `AI/Roundup/` > `AI/Events/` > `_LifeOS_/Journal/`
- `already_used: true` 제외

**형식 결정 (Reply 우선)**:
- **Reply (댓글)**: 커뮤니티 스레드에 직접 답할 수 있으면 최우선 — 가장 반응형
  - 단, **내가 쓴 포스트(own post)에는 reply를 달지 말 것**. (어색하고 스팸처럼 보일 수 있음)
  - 판정 기준: `primaryVault.slug == self_vault_slug` (이 값은 실행 전에 사용자에게 확인)
  - own post라면 Reply 대신 **New Thread/Brain Update**로 전환하거나 SKIP
...
python3 _LifeOS_/Skills/gobi-social-agent/scripts/update_state.py --kind bu --source ... --draft ...
```

## Heartbeat invocation (OpenClaw, v0.5.0+)

OpenClaw가 매 heartbeat tick마다 이 스킬을 자동 호출. 설정:
- `_Claw_/home/openclaw.json#skills.load.extraDirs`에 `_LifeOS_/Skills`가 등록되어 있어야 함
- `_Claw_/HEARTBEAT.md`에 GSA 태스크 블록 존재
- (테스트 모드) 2시간 cooldown/throttle 비활성화 — heartbeat가 30분마다 호출해도 쿨다운 때문에 SKIP하지 않음
...
```
_LifeOS_/Skills/gobi-social-agent/
├── SKILL.md
├── scripts/
│   ├── collect_signals.py
│   ├── update_state.py
...
# Back-symlink in _Settings_/Skills/gobi-social-agent/ → _LifeOS_/Skills/gobi-social-agent/
